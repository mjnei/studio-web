      ...
      <div className="max-h-[90vh] w-full max-w-lg overflow-y-auto rounded-2xl border border-border-default bg-surface-panel shadow-xl">
        <div className="flex items-start justify-between border-b border-border-default px-5 py-4">
          <div>
            ...
            <p className="mt-1 text-sm text-text-muted">Project ID {project.id}</p>
          </div>
          <button
            type="button"
            onClick={onClose}
        ...
        <div className="space-y-4 px-5 py-4 text-sm">
          <div className="grid grid-cols-2 gap-3">
            <div>
              <p className="text-xs uppercase tracking-wider text-text-muted">Owner</p>
            ...
            <div>
              <p className="text-xs uppercase tracking-wider text-text-muted">Movie</p>
              <p className="mt-1 text-text-primary">{project.movie?.title || "—"}</p>
            </div>
            <div>
              <p className="text-xs uppercase tracking-wider text-text-muted">Step</p>
          ...
          {project.active_tts_job && (
            <div>
              <p className="text-xs uppercase tracking-wider text-text-muted">Active TTS Job</p>
              <p className="mt-1 text-text-primary">
            ...
            <div className="rounded-xl border border-border-default bg-surface-raised p-4">
              ...
              <div className="flex flex-wrap items-center gap-2">
                <select
                  ...
                  className="rounded-lg border border-border-default bg-surface-panel px-3 py-2 text-sm text-text-primary"
                >
                  {STATUSES.map((s) => (
                ...
                <button
                  ...
                  className="rounded-lg bg-accent-primary px-4 py-2 text-sm font-medium text-white disabled:opacity-50"
                >
        ...
        <div className="flex justify-end gap-2 border-t border-border-default px-5 py-4">
          ...
          <button
            type="button"
            onClick={onClose}
            className="rounded-lg border border-border-default px-4 py-2 text-sm text-text-primary hover:bg-surface-hover"
          >
            Close
          </button>
        </div>
