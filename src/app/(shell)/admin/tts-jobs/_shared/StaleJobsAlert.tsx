...
interface StaleJobBase {
  id: number;
  job_id: string;
  status: string;
}

interface StaleJobsAlertProps<T extends StaleJobBase> {
  staleJobs: T[];
  title: string;
  description: string;
  onCancel?: (jobId: string) => void;
  renderMetadata: (job: T) => ReactNode;
  renderPreview?: (job: T) => ReactNode;
}

export function StaleJobsAlert<T extends StaleJobBase>({
  staleJobs,
  title,
  description,
  onCancel,
            ...
            {staleJobs.map((job) => (
              ...
              >
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-caption text-text-secondary">{renderMetadata(job)}</p>
                  {renderPreview?.(job)}
                </div>
                {onCancel && (
                  <button
