...
/**
 * Admin TTS Jobs Client
 * Provides functions for monitoring and managing TTS job health.
 */
...
export async function retryTTSJob(jobId: number): Promise<TTSJob> {
  return request<TTSJob>(`/admin/tts-jobs/${jobId}/retry`, {
    method: "POST",
  });
}
...
export async function cancelTTSJob(jobId: number): Promise<void> {
  return request<void>(`/admin/tts-jobs/${jobId}/cancel`, {
    method: "POST",
  });
}
